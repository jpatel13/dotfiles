...
myStartupHook = do
  ...
  spawnOnce "lxsession"
  spawnOnce "picom --config ~/.config/picomjonaburg.conf"
  spawnOnce "nm-applet"
  ...
  spawnOnce "feh --randomize --bg-fill ~/.resources/wallpapers/*.jpg"  -- feh set random wallpaper
...
myKeys c = (subtitle "Custom Keys":) $ mkNamedKeymap c $
  ...
  -- Override dmenu with Rofi 
  , ("M-S-p", addName "" $ spawn "rofi -show run")
...
-- Useful programs to have a keybinding for launch
  ...
  , ("M-u <Space>", addName "mocp toggle pause"  $ spawn "mocp --toggle-pause")-- Emacs (SUPER-e followed by a key)
...
main = do
  ...
  xmproc0 <- spawnPipe ("xmobar -x 0 ~/.xmonad/gruvbox-dark-xmobarrc")
